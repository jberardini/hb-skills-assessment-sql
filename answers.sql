...
SELECT b.name FROM brands AS b JOIN models AS m ON b.name=m.brand_name GROUP BY b.name HAVING COUNT(*)>5;
...
SELECT m.name, m.year FROM models AS m FULL OUTER JOIN brands AS b ON m.brand_name=b.name WHERE m.year IN (SELECT founded FROM brands);